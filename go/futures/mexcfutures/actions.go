...
func registerAllActions(m map[string]actionHandler) {
	...
	m["contract_detail_country"] = handleContractDetailCountry
	...
	m["submit_order"] = handleCreateOrder // backward-compatible alias
...
func handleContractDetailCountry(ctx context.Context, c *FuturesRestClient, params map[string]any) (*RawResponse, error) {
	symbol := paramStr(params, "symbol", "")
	return c.GetContractDetailCountry(ctx, symbol)
}
...
func handleStpSearch(ctx context.Context, c *FuturesRestClient, params map[string]any) (*RawResponse, error) {
	return c.GetStpSearch(ctx)
}
